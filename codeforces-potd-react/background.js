...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.status === 'loading' && tab.url) {
    ...
    // Check if restriction is enabled and today's POTD is solved
    chrome.storage.local.get(['potdSolvedDate', 'potdRestrictionEnabled'], (localResult) => {
      const today = new Date().toISOString().slice(0, 10);
      if (!localResult.potdRestrictionEnabled) return;
      if (localResult.potdSolvedDate === today) return;
      // Get current POTD info
      chrome.storage.sync.get(['dailyProblem', 'lastFetchDate'], (syncResult) => {
        if (syncResult.lastFetchDate === today && syncResult.dailyProblem) {
          const potd = syncResult.dailyProblem;
          const potdUrl = `https://codeforces.com/contest/${potd.contestId}/problem/${potd.index}`;
          const blockUrl = chrome.runtime.getURL('potd_block.html') + '?potdUrl=' + encodeURIComponent(potdUrl);
